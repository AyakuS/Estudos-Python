...
então calcular e mostrar o valor do troco. Após esta operação, o programa deverá voltar ao ponto inicial,
...
while opcao != 0:
    count += 1
    deseja = ""
    item = float(input(f"Produto {count}: "))
    valor_total += item
    opcao = int(input("Existem mais produtos? Digite [1] - Sim e [0] - Não\n"))
    
print(f'Total a pagar = {valor_total}')
pagamento = float(input('Pagamento: '))
print(f'Troco: {pagamento - valor_total}')
